	...
	update_tx_db2:function(hash){
		  Address.findOne({a_id: hash}, function(err, add) {
                          ...
                          if(add)
			  {
				...
				var reces=AddressTx. aggregate([
							    {
							        $match: {
							            "a_id": add.a_id  // 添加条件筛选
							        }
							    },
							    {
							        $group: {
							            _id: null,  // 不分组，汇总所有记录
							            totalAmount: { $sum: "$amount" }  // 汇总amount字段
							        }
							    }
							]);
				  ...
				  var recieves=0;
				   var rssum=0;
				  if(rs.length>0)
					 rssum= rs[0].totalAmount;
				  if(pays.length>0)
					  paysum=pays[0].totalAmount;
				  if(reces.length>0)
					  recieves=reces[0].totalAmount
				 			 
				  add.balance=rssum+recieves-paysum;
				  add.received=rssum+recieves;
				  Address.updateOne(
				    { "a_id": add.a_id },  // 查询条件
				    { 
				        $set: { 
