...
async function updateBalances(a_id) {  
 
        // 计算 vout.amount 的总和
        const voutResult = await Tx
          .aggregate([
		  { $match: { "vout.addresses": a_id } },
            { $unwind: "$vout" },          
            { $group: { _id: null, totalVout: { $sum: "$vout.amount" } } },
          ])
          .toArray();
 
        const totalVout = voutResult.length > 0 ? voutResult[0].totalVout : 0;
        // 计算 vin.amount 的总和
        const vinResult = await Tx
          .aggregate([
	  { $match: { "vin.addresses": a_id } },
            { $unwind: "$vin" },          
            { $group: { _id: null, totalVin: { $sum: "$vin.amount" } } },
          ])
          .toArray();
 
        const totalVin = vinResult.length > 0 ? vinResult[0].totalVin : 0;
        // 计算新的 balance
        const newBalance = totalVout - totalVin;

        // 更新 addresses 表的 balance
        await Address.updateOne(
          { a_id: a_id },
          { $set: { balance: newBalance,received:totalVout } }
        );

        console.log(`Updated balance for ${a_id}: ${newBalance}`);     
...
function is_locked(lockfile, cb) {
 ...
 function update_tx_db2(hash){

		 try{
    

		  Address.findOne({a_id: hash}, function(err, add) {
    
 const now = new Date(); // 获取当前时间
console.log(now.toISOString()); // 输出当前时间，格式为 ISO 8601，精确到毫秒
                          if(add)
			  {
				    var paysum=0;
				  var recieves=0;
				   var rssum=0;
				var pays=Tx.aggregate([
						    {
						        $unwind: "$vin"  // 将vin数组展开为多条记录
						    },
						    {
						        $match: {
						            "vin.addresses": add.a_id  // 添加条件筛选
						        }
						    },
						    {
						        $group: {
						            _id: null,  // 不分组，汇总所有记录
						            totalAmount: { $sum: "$vin.amount" }  // 汇总amount字段
						        }
						    }
						],function (err,balance_sum1) {
						const now = new Date(); // 获取当前时间
					console.log(now.toISOString()); // 输出当前时间，格式为 ISO 8601，精确到毫秒
					          if (err) {
					            console.log("this is update error："+ err.message);
					          } else {
							  if(balance_sum1.length>0)
							  {
								  paysum = balance_sum1[0].totalAmount; // 处理每条记录 
							  }
							  
					  var rs=Tx.aggregate([
						    {
						        $unwind: "$vout"  // 将vin数组展开为多条记录
						    },
						    {
						        $match: {
						            "vout.addresses": add.a_id  // 添加条件筛选
						        }
						    },
						    {
						        $group: {
						            _id: null,  // 不分组，汇总所有记录
						            totalAmount: { $sum: "$vout.amount" }  // 汇总amount字段
						        }
						    }
						],function (err,balance_sum2) {
						  const now = new Date(); // 获取当前时间
						console.log(now.toISOString()); // 输出当前时间，格式为 ISO 8601，精确到毫秒
					          if (err) {
					            console.log("this is update error："+ err.message);
					          } else {
							   if(balance_sum2.length>0)
							  {
								  rssum = balance_sum2[0].totalAmount; // 处理每条记录 
							  }
							  
				
											  	  
								  add.balance=rssum-paysum;
								  add.received=rssum;
							   console.log(rssum);
							   console.log(recieves);
								  Address.updateOne(
								    ...
								    { 
								        $set: { 
								            balance:  add.balance,  // 新的balance值
								            received:  add.received   // 新的received值
								        } 
								    },function() {
							  ...
							  const now = new Date(); // 获取当前时间
					console.log(now.toISOString()); // 输出当前时间，格式为 ISO 8601，精确到毫秒
								 console.log("this is update:"+ add.a_id);
  				  ...
  				  console.warn('this update address error:'+e.message);
...
module.exports = {
    ...
    update_richlist: function (list, cb) {
        if (list == 'received') {
            Address.find({}, 'a_id balance received name').sort({ received: 'desc' }).limit(1000).exec(function (err, addresses) {
            ...
            Address.find({}, 'a_id balance received name').sort({ balance: 'desc' }).limit(1000).exec(function (err, addresses) {
                Richlist.updateOne({ coin: settings.coin }, {
                    balance: addresses,
                }, function () {
                    return cb();
    ...
    get_last_txs_ajax: function (start, length, min, cb) {
        ...
        Tx.find({ 'blockindex': { $gte: 498888 } }).countDocuments(function (err, count) {
            Tx.find({'blockindex': { $gte: 498888 } }).sort({ blockindex: -1 }).skip(Number(start)).limit(Number(length)).exec(function (err, txs) {
...
//            Tx.find({'total': { $gte: min } }).sort({ blockindex: -1 }).skip(Number(start)).limit(Number(length)).exec(function (err, txs) {
    ...
    get_distribution: function (richlist, stats, cb) {
      ...
      if (count <= 1000 && count > 100) {
                ...
                distribution.t_101_1000.total = distribution.t_101_1000.total + (richlist.balance[i].balance / 100000000);
            }


            loop.next();
        }, function () {
            distribution.t_1001plus.percent = parseFloat(100 -  distribution.t_101_1000.percent- distribution.t_76_100.percent - distribution.t_51_75.percent - distribution.t_26_50.percent - distribution.t_1_25.percent).toFixed(2);
            distribution.t_1001plus.total = parseFloat(distribution.supply - distribution.t_101_1000.total - distribution.t_76_100.total - distribution.t_51_75.total - distribution.t_26_50.total - distribution.t_1_25.total).toFixed(8);
            ...
            distribution.t_101_1000.total = parseFloat(distribution.t_101_1000.total).toFixed(8);
