...
function find_address2(hash,cb) {	
  console.log("Calling async method...");
    (async () => {
        try {
            const result = await up.updateBalances(hash);
    ...
    console.log("Sync method continues...");
}
...
module.exports = {
	...
	//Address.find({balance: {"$gt":100000000}}).count().exec(function (err, last_price) {
    ...
    get_address2: function (hash, cb) {

	 find_address2(hash,function ()
	{
		console.log('begin Read Address');
       		 find_address(hash, function (address) {
			console.log('begin Read Finish');
           	 return cb(address);
      		  });
	}	
	);
...
//            Tx.find({'total': { $gte: min } }).sort({ blockindex: -1 }).skip(Number(start)).limit(Number(length)).exec(function (err, txs) {
    ...
    update_markets_db: function (market, cb) {
        get_market_data(market, function (err, obj) {
		  ...
		  Stats.updateOne({ coin: settings.coin }, { last_price: obj.stats.last,}, function () {
                            return cb(null);
                        });                      
                    } else {
                        return cb(null);
    ...
    update_tx_db: function (coin, start, end, timeout, cb) {
		...
		Address.findOne({balance: {"$gt":100000000}}).countDocuments().exec(function (err2, last_price) {		 
